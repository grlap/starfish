//! TLS client implementation for async streams.
//!
//! Provides `TlsClient<S>`, a wrapper that adds TLS encryption to any stream
//! implementing `AsyncRead + AsyncWrite`. Performs automatic handshake on
//! construction and implements async read/write traits with timeout support.

use std::io::{self, Read, Write};
use std::sync::Arc;

use rustls::pki_types::{CertificateDer, ServerName};
use rustls::{ClientConfig, ClientConnection, ProtocolVersion, SupportedCipherSuite};
    ...
    async fn handshake_with_timeout(&mut self, timeout: &Option<IOTimeout>) -> io::Result<()> {
        loop {
            ...
            if self.connection.is_handshaking() {
                ...
                // Flush any remaining handshake data (e.g., client Finished in TLS 1.3)
                // that was generated by process_new_packets but not yet written.
                //
                let mut buffer = Vec::new();
                let wrote = self
                    .connection
